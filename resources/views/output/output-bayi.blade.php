...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .hospital-name { font-size: 17px; font-weight: 800; color: #00A1C5; letter-spacing: -.02em; text-transform: uppercase; line-height: 1; margin-bottom: 4px; }
        ...
        /* ── RULE ─────────────────────────────────────── */
        .double-rule { margin: 8px 0 10px; border: none; border-top: 3px solid var(--ink); box-shadow: 0 2px 0 var(--ink); height: 5px; }
        ...
        .doc-title { font-size: 13px; font-weight: 800; text-transform: uppercase; letter-spacing: .07em; line-height: 1.2; }
            ...
            <table class="form-table">
                <tbody>
                    ...
                    <tr>
                        ...
                        <td colspan="3" style="font-weight:700;text-transform:uppercase">{{ $certificate->data['nama_lengkap'] ?? 'BY. NY. ' . ($certificate->data['nama_ibu'] ?? '-') }}</td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        ...
                        <td>{{ $certificate->data['jam_lahir'] ?? '-' }} WIB</td>
                    </tr>
                    <tr>
        ...
        <div class="grid-2">
            ...
            <div>
                ...
                <div class="age-display">
                    <span class="age-val">
                        @if(isset($certificate->data['umur_hari']) && $certificate->data['umur_hari'] > 0)
                            {{ $certificate->data['umur_hari'] }}
                        @else
                            {{ $certificate->data['umur_jam'] ?? '0' }}
                        @endif
                    </span>
                    <span class="age-unit">
                        @if(isset($certificate->data['umur_hari']) && $certificate->data['umur_hari'] > 0)
        ...
        <div class="grid-2">
            <div>
                ...
                <div class="info-panel" style="height:68px;display:flex;flex-direction:column;justify-content:center;gap:3px">
                    <div class="kv"><span class="k">Nama Ibu</span><span>{{ $certificate->data['nama_ibu'] ?? '-' }}</span></div>
                    <div class="kv"><span class="k">Umur Ibu</span><span>{{ $certificate->data['umur_ibu'] ?? '-' }} Tahun</span></div>
                    <div class="kv"><span class="k">Usia Gestasi</span><span>{{ $certificate->data['usia_kehamilan_minggu'] ?? '-' }} Minggu</span></div>
                </div>
            </div>
            <div>
                <div class="sec-head-light">5. Spesifikasi Lahir &amp; Kematian</div>
                <div class="info-panel" style="height:68px;display:flex;flex-direction:column;justify-content:center;gap:3px">
                    <div class="kv"><span class="k">Berat Lahir</span><span>{{ $certificate->data['berat_lahir_gram'] ?? '-' }} gram</span></div>
                    <div class="kv"><span class="k">Jenis Kelahiran</span><span>{{ $certificate->data['jenis_persalinan'] ?? '-' }}</span></div>
                    <div>
                        <span style="font-weight:600;color:var(--ink-mid);font-size:9px">Meninggal Saat: </span>
                        <span class="cb {{ isset($certificate->data['mati_antepartum']) && $certificate->data['mati_antepartum'] ? 'on' : '' }}"></span><span style="font-size:8.5px">Sebelum Lahir &nbsp;</span>
                        <span class="cb {{ isset($certificate->data['mati_intrapartum']) && $certificate->data['mati_intrapartum'] ? 'on' : '' }}"></span><span style="font-size:8.5px">Saat Lahir</span>
                    </div>
                </div>
            </div>
        </div>
            ...
            <table class="form-table">
                <thead>
                    <tr style="background:var(--bg-cell);text-align:center;font-size:8.5px;font-weight:700">
                        <td style="width:65%">Kategori Kondisi Utama Utama / Kontributor</td>
                        <td style="width:20%">Interval / Selang Waktu</td>
                        <td style="width:15%">Kode ICD-10</td>
                    </tr>
                </thead>
                <tbody>
                    <tr>
                        <td><b style="color:var(--ink-mid);margin-right:4px">a.</b> Kondisi Utama Bayi (Main disease/condition in fetus or infant):<br><span style="padding-left:14px;font-weight:500;">{{ $certificate->data['penyebab_a'] ?? '-' }}</span></td>
                    ...
                    <tr>
                        <td><b style="color:var(--ink-mid);margin-right:4px">b.</b> Kondisi Lain Bayi (Other diseases/conditions in fetus or infant):<br><span style="padding-left:14px;font-weight:500;">{{ $certificate->data['penyebab_b'] ?? '-' }}</span></td>
                    ...
                    <tr>
                        <td><b style="color:var(--ink-mid);margin-right:4px">c.</b> Kondisi Utama Ibu mempengaruhi janin (Main maternal disease affecting fetus):<br><span style="padding-left:14px;font-weight:500;">{{ $certificate->data['penyebab_c'] ?? '-' }}</span></td>
                    ...
                    <tr>
                        <td><b style="color:var(--ink-mid);margin-right:4px">d.</b> Kondisi Lain Ibu mempengaruhi janin (Other maternal diseases affecting fetus):<br><span style="padding-left:14px;font-weight:500;">{{ $certificate->data['penyebab_d'] ?? '-' }}</span></td>
